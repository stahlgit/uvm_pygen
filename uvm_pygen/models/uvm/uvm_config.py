...
class UVMConfiguration:
    ...
    def _parse(self):
        ...
        self.components = [
            Component(**c) for c in env.get('components', [])
        ]
        
        # Transaction
        trans = self._raw_config.get('transaction', {})
        ...
        # Sequences
        self.sequences = [
            Sequence(**s) for s in self._raw_config.get('sequences', [])
        ]
        ...
        # Tests
        self.tests = [
            Test(**t) for t in self._raw_config.get('tests', [])
        ]
